...
2. **全管理者取得**
    ...
    - セッションから現在ログイン中の管理者 ID を取得
    - `SystemAdminDAO.selectAllExceptId()` で自分自身を除外して管理者を取得
    - 論理削除されていない管理者のみ取得（`deleted_at IS NULL`）
    - 取得順は作成日時順（デフォルト）
    - **自分自身のアカウントは一覧に表示されない**

3. **リスト変換**

    - DTO リストを `ArrayList` に変換
    - JSP で安全に扱えるようにする
...
-   **SystemAdminDAO**
    -   `selectAllExceptId(UUID excludeId)`: 指定 ID を除外してシステム管理者を取得
        -   SQL:
            ```sql
            ...
            ```
        -   パラメータ: `excludeId` - 除外する管理者 ID（現在ログイン中の管理者 ID）

### セキュリティ

-   **論理削除フィルタ**: 削除済み管理者は表示されない
-   **自分自身除外**: 現在ログイン中の管理者は一覧に表示されない
-   **セッションチェック**: FrontController で管理者権限確認済み
...
-   データが 0 件の場合でも正常に表示（空リスト）
-   初期データとして 10 件の管理者が存在（`DatabaseInitListener`）
...
-   **自分自身のアカウントは一覧から除外されるため、誤って自分自身を削除することを防止**
