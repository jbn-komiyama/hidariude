                ...
                <c:choose>
                    ...
                    <c:forEach var="ct" items="${contacts}" varStatus="cs">
                      <tr>
                        <c:if test="${cs.first}">
                          ...
                          <td class="text-nowrap" rowspan="${rowspan}"><c:out value="${cst.companyCode}"/></td>
                          <td class="text-nowrap" rowspan="${rowspan}"><c:out value="${cst.companyName}"/></td>
                        </c:if>

                        <!-- 担当者名だけ縦展開 -->
                        <td class="text-nowrap">
                          <c:out value="${ct.name}"/>
                          <c:if test="${not empty ct.nameRuby}">
                        ...
                        <c:if test="${cs.first}">
                          <%-- 連絡先は顧客単位でrowspan --%>
                          <td class="text-break" rowspan="${rowspan}">
                            <c:if test="${not empty cst.mail}">
                              <a href="mailto:${cst.mail}">${cst.mail}</a><br/>
                            </c:if>
                            <c:out value="${cst.phone}"/><br/>
                            <c:if test="${not empty cst.postalCode}">〒<c:out value="${cst.postalCode}"/><br/></c:if>
                            <c:out value="${cst.address1}"/> <c:out value="${cst.address2}"/><br/>
                          ...
                          <td class="text-center" rowspan="${rowspan}">
                            <div class="d-flex gap-1 justify-content-center flex-wrap">
                              <form method="post" action="<%= request.getContextPath() %>/admin/customer/edit" class="m-0">
                                ...
                                <button type="submit" class="btn btn-sm btn-primary">顧客編集</button>
                              </form>
                              <form method="post" action="<%= request.getContextPath() %>/admin/customer/delete" class="m-0"
                                    onsubmit="return confirm('本当に削除しますか？');">
                                <input type="hidden" name="id" value="${cst.id}">
                                <button type="submit" class="btn btn-sm btn-danger">顧客削除</button>
                              </form>
                              <form method="post" action="<%= request.getContextPath() %>/admin/contact" class="m-0">
                                <input type="hidden" name="customerId" value="${cst.id}">
                                <button type="submit" class="btn btn-sm btn-outline-primary">担当者管理</button>
                              </form>
                            </div>
                          </td>
                        </c:if>
                  ...
                  <c:otherwise>
                    <tr>
                      ...
                      <td class="text-nowrap"><c:out value="${cst.companyCode}"/></td>
                      <td class="text-nowrap"><c:out value="${cst.companyName}"/></td>
