...
<body class="bg-primary bg-opacity-10">
  ...
  <div class="container py-4">
    ...
    <div class="card p-3 shadow-sm">
      ...
      <!-- 戻る（編集へ） -->
      <form method="post" action="<%= request.getContextPath() %>/admin/secretary/edit" class="d-inline">
        <input type="hidden" name="id" value="${id}">
        <c:forEach var="n" items="${['secretaryCode','secretaryRankId','pmSecretary','name','nameRuby','mail','phone','postalCode','address1','address2','building']}">
      ...
      <!-- 確定 -->
      <form method="post" action="<%= request.getContextPath() %>/admin/secretary/edit_done" class="d-inline">
        <input type="hidden" name="id" value="${id}">
        <c:forEach var="n" items="${['secretaryCode','secretaryRankId','pmSecretary','name','nameRuby','mail','phone','postalCode','address1','address2','building']}">
          <input type="hidden" name="${n}" value="${requestScope[n]}">
        </c:forEach>
