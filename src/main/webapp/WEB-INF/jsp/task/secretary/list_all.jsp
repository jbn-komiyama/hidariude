...
<%@ include file="/WEB-INF/jsp/_parts/secretary/navbar.jspf" %>
	<div class="container py-4">
		...
		<ul class="nav nav-tabs mb-3">
			<li class="nav-item"><a class="nav-link active" href="${urlAll}">すべて</a></li>
		...
		<c:choose>
			<c:when test="${empty tasks}">
				<div class="p-4 text-center text-muted">この月の業務はありません。</div>
			</c:when>
			<c:otherwise>
								...
								<tr>
									...
									<td><c:choose>
											<c:when test="${t.remandedAt ne null}">
												<span class="badge bg-danger text-white">差戻</span>
											</c:when>
											<c:when test="${t.approvedAt eq null and t.hasRemander}">
												<span class="badge bg-danger text-white"
													title="${fn:escapeXml(t.remandComment)}">差戻修正</span>
											</c:when>
										...
										<div class="d-flex gap-1">
											<form method="get"
												...
												<button type="submit"
													...
													<c:if test="${t.approvedAt ne null}">disabled title="承認済みは編集できません"</c:if>>編集</button>
											</form>
											<form method="post"
												...
												<button type="submit" class="btn btn-sm btn-outline-danger"
													<c:if test="${t.approvedAt ne null}">disabled title="承認済みは削除できません"</c:if>>削除</button>
											</form>
										</div>
									</td>
								</tr>
