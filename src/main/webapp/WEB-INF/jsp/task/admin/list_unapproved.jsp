...
<div class="container py-4">
  ...
  <ul class="nav nav-tabs mb-3">
    ...
    <li class="nav-item"><a class="nav-link active" href="${urlUnapp}">未承認</a></li>
    <li class="nav-item"><a class="nav-link" href="${urlApp}">承認済</a></li>
    <li class="nav-item"><a class="nav-link" href="${urlRemand}">差戻</a></li>
  </ul>

  <!-- フィルタ -->
  <form method="get" action="<%=request.getContextPath()%>/admin/task/list_unapproved" class="card card-body shadow-sm mb-3">
  ...
  <div class="card shadow-sm">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
      <span class="fw-semibold">未承認（${yearMonth}）</span>
      <!-- 右上ボタン -->
      <form id="approveFormTop" class="m-0" onsubmit="document.getElementById('approveForm').requestSubmit(); return false;">
        <button type="submit" class="btn btn-sm btn-primary">選択を一括承認</button>
      </form>
    </div>
    ...
    <div class="card-body p-0">
      <c:choose>
        ...
        <c:otherwise>
          ...
          <form id="approveForm" method="post" action="<%=request.getContextPath()%>/admin/task/approve_bulk" class="m-0">
                ...
                <tbody>
                  <c:forEach var="t" items="${tasks}" varStatus="st">
                    <tr>
                      ...
                      <td>
                        <div class="form-check m-0">
                          <input class="form-check-input row-check" type="checkbox" name="taskIds" value="${t.id}" checked>
                        </div>
                      </td>
                      <td>${st.count}</td>
...
(() => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (n === 0) { e.preventDefault(); alert('承認対象が選択されていません。'); return; }
    if (!confirm(n + '件を一括承認します。よろしいですか？')) e.preventDefault();
  });
})();
</script>
</body>
