...
　　<!--    ページネーション-->
    ...
    <div class="card shadow-sm">
			    ...
			    <c:choose>
			      <c:when test="${not empty groups}">
			        ...
			        <c:forEach var="g" items="${groups}">
			          <c:set var="customerRowspan" value="${customerRowspan + fn:length(g.assignments)}" />
			        </c:forEach>
			          ...
			          <c:forEach var="a" items="${g.assignments}" varStatus="as">
			            <tr>
			              <c:if test="${gs.first && as.first}">
							  <td class="text-center fw-semibold" rowspan="${customerRowspan}">
							    ${st.index + 1 }
							  </td>
							  ...
							  <td class="text-nowrap" rowspan="${customerRowspan}">
							    ...
							    <c:set var="hasPm" value="false"/>
							    <c:forEach var="g2" items="${groups}">
			              ...
			              <c:if test="${as.first}">
			                <td class="text-nowrap" rowspan="${fn:length(g.assignments)}">
			                  <c:out value="${g.secretary != null ? g.secretary.name : '—'}"/>
			                </td>
			                <td class="text-nowrap" rowspan="${fn:length(g.assignments)}">
			                  <c:out value="${g.secretary != null && g.secretary.secretaryRank != null ? g.secretary.secretaryRank.rankName : '—'}"/>
			                </td>
			              </c:if>
			              ...
			              <td class="text-nowrap text-end">
			                <fmt:formatNumber value="${a.increaseBasePayCustomer}" pattern="#,##0" />
			                (<fmt:formatNumber value="${a.increaseBasePaySecretary}" pattern="#,##0" />)
			              </td>
      ...
      <div class="card-footer text-end small text-muted">
        件数：<span class="fw-semibold"><c:out value="${fn:length(assignments)}"/></span>
      </div>
    </div>
  </div>
<!--  ページネーション-->
