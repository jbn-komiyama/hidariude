...
public class InvoiceService extends BaseService {
	...
	public void issueInvoiceExcel(HttpServletResponse resp) {
		...
		HttpSession session = req.getSession(false);
		if (session == null)
			throw new ServiceException("E:INV-ISSUE00 セッションが無効です。");
		LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");
		...
		try (InputStream in = req.getServletContext().getResourceAsStream(TEMPLATE_PATH)) {
			...
			try (XSSFWorkbook wb = new XSSFWorkbook(in)) {
				...
				setCellFormula(sh, subRow, 10,
						"SUM(J" + TAXABLE_DATA_START + ":J" + (TAXABLE_DATA_START + count - 1) + ")");
	...
	public void issueInvoiceExcel2(HttpServletResponse resp) {
		...
		try (InputStream in = req.getServletContext().getResourceAsStream(TEMPLATE_PATH)) {
			...
			for (InvoiceDTO inv : invoiceDtos) {
				if (inv.getFee() != null)
					grandTotalFinal = grandTotalFinal.add(inv.getFee());
			}
			int totalMinutesFinal = 0;
			for (TaskDTO t : taskDtos) {
				if (t.getWorkMinute() != null)
					totalMinutesFinal += t.getWorkMinute();
			}
	...
	public String customerInvoiceSummary() {
		...
		HttpSession session = req.getSession(false);
		if (session == null)
			throw new ServiceException("E:INV-C00 セッションが無効です。");
		LoginUser lu = (LoginUser) session.getAttribute("loginUser");
		UUID customerId = null;
		if (lu != null) {
			if (lu.getCustomer() != null)
				customerId = lu.getCustomer().getId();
			if (customerId == null && lu.getCustomerContact() != null) {
				customerId = lu.getCustomerContact().getCustomerId();
			}
		}
		if (customerId == null)
			throw new ServiceException("E:INV-C01 顧客IDが取得できません。");
		...
		if (taskDtos != null) {
			for (TaskDTO t : taskDtos) {
				...
				try {
					...
					if (!approved)
						try { // getApprovalStatus(): String -> "APPROVED" / "承認済み"
