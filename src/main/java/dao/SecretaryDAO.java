...
public class SecretaryDAO extends BaseDAO {
    ...
    private static final String SQL_DELETE_LOGICAL =
	        "UPDATE secretaries SET deleted_at = CURRENT_TIMESTAMP WHERE id = ?";

    private static final String SQL_COUNT_BY_CODE =
	        "SELECT COUNT(*) FROM secretaries WHERE secretary_code = ?";

    private static final String SQL_COUNT_BY_MAIL =
	        "SELECT COUNT(*) FROM secretaries WHERE mail = ?";

    private static final String SQL_SELECT_RANK_ALL =
	        "SELECT id, rank_name, description, "
	      + "       increase_base_pay_customer, increase_base_pay_secretary, "
	      + "       created_at, updated_at, deleted_at "
	      + "  FROM secretary_rank";
    
    // 自ID除外つきの重複チェック用 SQL
    private static final String SQL_COUNT_BY_CODE_EXCEPT_ID =
            "SELECT COUNT(*) FROM secretaries WHERE secretary_code = ? AND id <> ?";

    // 自ID除外つきの重複チェック用 SQL（メール）
    private static final String SQL_COUNT_BY_MAIL_EXCEPT_ID =
            "SELECT COUNT(*) FROM secretaries WHERE mail = ? AND id <> ?";
	...
	/**
     * 削除されていない秘書（secretaries）をランク情報とともに全件取得します。
     *
     * @return {@link SecretaryDTO} のリスト（0件時は空）
	...
	/**
     * PM 対応可能（{@code is_pm_secretary = TRUE}）かつ削除されていない秘書を取得します。
     *
     * @return {@link SecretaryDTO} のリスト（0件時は空）
     * @throws DAOException DBアクセスに失敗した場合
     */
     ...
     * 秘書ランクを全件取得します。
     *
     * @return {@link SecretaryRankDTO} のリスト（0件時は空）
     * @throws DAOException DBアクセスに失敗した場合
     */
     ...
     */
    public boolean secretaryCodeCheckExceptId(String code, UUID excludeId) { // ★ ADDED
     ...
     */
    public boolean mailCheckExceptId(String mail, UUID excludeId) { // ★ ADDED
    ...
    /**
     * {@code secretary_rank} のカラム群を {@link SecretaryRankDTO} に詰めます。
     *
     * @param rs  ResultSet
     * @param num 開始カラム（1-origin）
     * @return {@link SecretaryRankDTO}
